...
### Bot Management 핵심 기술: 챌린지(Challenge) 방식

Bot Management가 봇과 사람을 구별하는 핵심은 **클라이언트에게 JavaScript 실행을 요구**하고 그 결과를 분석하는 챌린지 메커니즘입니다.

#### ① 브라우저 핑거프린팅 (Browser Fingerprinting)

브라우저와 디바이스 환경에서 수집한 여러 속성을 조합해 **고유한 식별값(Fingerprint)** 을 생성합니다. 봇은 실제 브라우저 환경을 완벽히 흉내 내기 어렵기 때문에 이상 신호가 나타납니다.

| 수집 항목 | 정상 브라우저 | 봇 (Headless Chrome 등) |
| :--- | :--- | :--- |
| `navigator.webdriver` | `undefined` | `true` ← 핵심 탐지 |
| Canvas 렌더링 결과 | OS/GPU마다 픽셀 값이 미세하게 다름 | 특정 패턴으로 수렴하거나 기본값 반환 |
| WebGL 렌더러 | 실제 GPU 정보 (예: NVIDIA GeForce) | 소프트웨어 렌더러 (SwiftShader, llvmpipe) |
| 설치된 플러그인 수 | 1개 이상 | 0개 |
| 브라우저 언어 설정 | `['ko', 'en-US', ...]` | `[]` (빈 배열) |

> **Canvas Fingerprinting 원리:** WAF가 삽입한 스크립트가 Canvas에 특정 텍스트/도형을 그리게 하고, 픽셀 데이터를 해시값으로 변환합니다. OS, GPU, 폰트 환경에 따라 렌더링 결과가 미세하게 달라지므로, Headless 봇처럼 그래픽 환경이 단순한 경우 이상한 해시값이 나와 탐지됩니다.

#### ② 마우스 이벤트 분석 (Mouse Event Analysis)

사람은 마우스를 움직일 때 곡선 경로, 자연스러운 속도 변화, 미세한 떨림이 나타납니다. 봇은 이를 재현하기 어렵습니다.

```
[사람의 마우스 이동]          [봇의 마우스 이동]
  시작점                       시작점
   ↓ ∿∿∿↗↘∿→                    ↓ ────────→
  끝점                         끝점
  - 곡선형, 속도 변화 있음      - 직선, 등속 이동
  - 클릭 전 미세 떨림 존재      - 정확히 중앙 좌표 클릭
  - mousedown → mouseup: 80~300ms  - mousedown → mouseup: 0~1ms
```

| 분석 항목 | 사람 | 봇 |
| :--- | :--- | :--- |
| 이동 경로 | 비선형 곡선 | 직선 (선형성 지수 ≈ 1.0) |
| 클릭 위치 | 버튼 중앙 근처, 약간의 오차 | 픽셀 단위로 정확히 중앙 |
| 클릭 유지 시간 | 80ms ~ 300ms (매번 다름) | 0ms 또는 항상 일정한 값 |
| 페이지 진입 후 첫 동작 | 잠시 후 마우스 이동 시작 | 즉시 클릭 (이동 없음) |

#### ③ 종합 위험 점수화 (Risk Scoring)

각 신호에 가중치를 부여해 합산한 뒤 임계값을 초과하면 차단 또는 CAPTCHA를 발동합니다.

```
Risk Score 예시:
  navigator.webdriver = true   → +40점
  마우스 이동 기록 없음         → +25점
  Canvas 핑거프린트 이상        → +20점
  클릭 유지 시간 0ms           → +15점
  ────────────────────────────────────
